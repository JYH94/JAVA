...
public class PageMaker {
	...
	private int spageNo; // View 에 표시할 첫PageNo : 계산필요
	private int epageNo; // View 에 표시할 끝PageNo : 계산필요
	// ※ 주의 필드명이 ePage 처럼 두번째 알파벳이 대문자인 경우
	//    => setter, getter 는 setsPageNo , getsPageNo 형태로 만들어지기때문에
	//       Lombok.. 등등 과 규칙이다르므로 사용시 불편 
	//         -> 그러므로 대.소문자 섞어사용시 주의. 
	private int lastPageNo; // 출력 가능한 마지막 PageNo : 계산필요
	...
	public void calcData() {
		...
		// => 요청받은 currPage 만약 11 인 경우
		// epageNo = (int)Math.ceil( 11/(double)3 ) * 3 = 12
		// spageNo = (12 - 3) + 1 = 10
		
		// 3.2) lastPageNo 계산 & epageNo 의 적합성 검사
		// 
		
		this.lastPageNo = (int)Math.ceil(this.totalRowsCount / (double)cri.getRowsPerPage());
		if ( this.epageNo > lastPageNo ) {
